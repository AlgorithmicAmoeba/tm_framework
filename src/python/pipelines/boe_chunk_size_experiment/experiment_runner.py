"""
Stage 3 of the BOE chunk-size experiment.

Runs the BOE topic models over the experiment's document/word embeddings (one
set per target_chunk_count), stores results in the experiment's own
``pipeline.boe_cse_*`` tables and records per-run timing.

This is a close adaptation of the production
``pipelines/boe_06_topic_models/experiment_runner.py``: it keeps the same
model-building, alignment and result-insert logic, but reads/writes the
experiment's dedicated tables (no production tables are touched) and records
timing for every train+get_topics run.

Running from the command line:

set -a && . .env && set +a
while true; do
uv run src/python/pipelines/boe_chunk_size_experiment/experiment_runner.py
rc=$?
if [ $rc -eq 0 ]; then
    break
fi
...
from pipelines.boe_chunk_size_experiment.common import (
    BOE_TOPIC_MODELS,
    MODELS_NEEDING_WORD_EMBEDDINGS,
    NUM_TOPICS_LIST,
    TARGET_RESULTS,
    store_timing_result,
)
...
def get_boe_topic_model_id(session: Session, model_name: str) -> int:
    query = text("""
        SELECT id FROM pipeline.boe_cse_topic_model
...
def fetch_boe_word_embeddings(
    ...
    query = text("""
        SELECT word, vector
        FROM pipeline.boe_cse_word_embedding
        WHERE corpus_name = :corpus_name
        AND source_model_name = :source_model_name
        AND algorithm = :algorithm
        AND target_dims = :target_dims
...
def run_experiments(target_results: int = TARGET_RESULTS) -> None:
    ...
    with get_session(config.database) as session:
        for corpus_name in corpora:
            combos = get_embedding_combos(session, corpus_name)
    ...
    for corpus_name in corpora:
        ...
        if not combos:
            logging.info("No embedding combos for corpus: %s", corpus_name)
            continue
                ...
                for num_topics in NUM_TOPICS_LIST:
                    ...
                    for iteration in range(1, iterations_to_run + 1):
                        try:
                            ...
                            with get_session(config.database) as session:
                                insert_query = text("""
                                    INSERT INTO pipeline.boe_cse_topic_model_corpus_result
